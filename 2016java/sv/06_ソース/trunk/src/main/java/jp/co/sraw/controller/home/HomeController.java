...
public class HomeController extends CommonController {
	...
	public String list(Model model, Locale locale) {
		...
		//1-2.個人用お知らせ情報取得
		List<UsInfoTbl> listUsInfo = homeServiceImpl.findAllUsInfoByUserKey(userInfo.getTargetUserKey());

		//1-3.組織。及び、ロール向けお知らせ情報取得
		List<CmInfoTbl> listCmInfo = homeServiceImpl.findAllCmInfoByPartyOrRoll(userInfo.getTargetPartyCode(), userInfo.getTargetRoleCode());

		//1-2、1-3で取得した情報をマージする。
		ObjectUtil objectUtil = new ObjectUtil();
		List<CmInfoDto> listCmInfoDto = new ArrayList<CmInfoDto>();
		for (UsInfoTbl usInfoTbl : listUsInfo){
			CmInfoDto cmInfoDto = new CmInfoDto();
			objectUtil.getObjectCopyValue(cmInfoDto, usInfoTbl);
			listCmInfoDto.add(cmInfoDto);
		}
		for (CmInfoTbl cmInfoTbl : listCmInfo){
			CmInfoDto cmInfoDto = new CmInfoDto();
