...
public abstract class PortfolioServiceImpl<T extends GyCommonTbl, F extends PortfolioForm, R extends GyRepository>
	...
	public List<F> getDtoList(List<T> list) {
		...
		for (int i = 0; i < list.size(); i++) {
			dtoList.add(getPortfolioForm(list.get(i)));
		}
		return dtoList;
	}
	...
	public List<T> findAll(UserInfo userInfo, F form) {
		...
		Specification<T> whereUserKey = StringUtil.isNull(form.getUserKey()) ? null : new Specification<T>() {
			@Override
			public Predicate toPredicate(Root<T> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
				return cb.equal(root.get("usUserTbl"), form.getUserKey());
			}
		};
	...
	public boolean delete(UserInfo userInfo, F form) {
		...
		try {
			if (form.getPageMode().equals(CommonConst.PAGE_MODE_DELETE)) {
				T entity = findOne(form.getGyosekiKey(), form.getUpdDateAsTimestamp());
				if (entity == null) {
	...
	public boolean updateAll(UserInfo userInfo, F form) {
		...
		try {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				usUserTbl.setUserKey(userInfo.getLoginUserKey());
	...
	public boolean update(UserInfo userInfo, F form) {
		...
		try {
			...
			if (form.getPageMode().equals(CommonConst.PAGE_MODE_EDIT)) {
				entity = findOne(form.getGyosekiKey(), form.getUpdDateAsTimestamp());
				if (entity == null) {
					throw new Exception();
				}
			}
	...
	public F findOne(UserInfo userInfo, F form) {
		...
		T gyPaperTbl = findOne(form.getGyosekiKey(), form.getUpdDateAsTimestamp());
		if (gyPaperTbl == null) {
			return null;
		}
	...
	public T findOne(String gyosekiKey, Timestamp updDate) {
		Specification<T> whereGyosekiKey = StringUtil.isNull(gyosekiKey) ? null : new Specification<T>() {
			@Override
			public Predicate toPredicate(Root<T> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
		...
		return (T) repository.findOne((Specification<T>) Specifications.where(whereGyosekiKey).and(whereUpdDate));
	}
