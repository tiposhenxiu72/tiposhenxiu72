...
public class CompetitionExcelHelper extends PortfolioExcelHelper<GyCompetitionForm> {
	...
	public void buildExcelDocument(PoiBook workbook, List<GyCompetitionForm> list) {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			// 研究分野
			workbook.changeValue(rowno, 5, form.getField());
			// 研究種目
			workbook.changeValue(rowno, 5, form.getCategory());
			// 代表者
			workbook.changeValue(rowno, 5, form.getAuthor());
			// 配分額(総額)
			workbook.changeValue(rowno, 5, form.getAmounttotal().toString());
			// 公開範囲
			workbook.changeValue(rowno, 6, form.getPublicFlag());
		}
	}
